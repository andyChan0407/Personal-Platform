...
// 系统进程识别（Memory Compression 等不应被当成「用户应用」）
assert.strictEqual(sys.isSystemProc('Memory Compression'), true, '内存压缩应判为系统进程');
assert.strictEqual(sys.isSystemProc('svchost'), true, 'svchost 应判为系统进程');
assert.strictEqual(sys.isSystemProc('lsass.exe'), true, '带 .exe 的系统进程也应识别');
assert.strictEqual(sys.isSystemProc('chrome'), false, 'chrome 不应判为系统进程');
assert.strictEqual(sys.isSystemProc(''), false, '空名不应判为系统进程');

// 聚合：系统进程打 system 标记 + 状态「系统」；普通应用不受影响
const aggProcs = sys.aggregate(
  [
    { name: 'Memory Compression', pid: 1, cpuSeconds: 0, memBytes: 1300 * 1024 * 1024, path: null },
    { name: 'chrome', pid: 2, cpuSeconds: 0, memBytes: 300 * 1024 * 1024, path: null },
    { name: 'svchost', pid: 3, cpuSeconds: 0, memBytes: 200 * 1024 * 1024, path: null },
  ],
  null,
  Date.now(),
  16,
...
const mcApp = aggProcs.apps.find((a) => a.rawName === 'Memory Compression');
assert.ok(mcApp && mcApp.system === true, '聚合应标记 Memory Compression 为系统进程');
assert.strictEqual(mcApp.status, '系统', '系统进程状态应为「系统」');
const svcApp = aggProcs.apps.find((a) => a.rawName === 'svchost');
assert.ok(svcApp && svcApp.system === true && svcApp.status === '系统', 'svchost 聚合同样应标为系统');
const chromeApp = aggProcs.apps.find((a) => a.rawName === 'chrome');
assert.ok(chromeApp && chromeApp.system === false && chromeApp.status !== '系统', 'chrome 不应被当作系统进程');
assert.ok(aggProcs.apps.length >= 3, '候选列表应保留全部条目（最终展示由渲染层过滤）');
...
const run = runAsync.then(async () => {
  ...
  // —— 应用占用弹层：默认隐藏系统进程，可一键显示并标「系统」（修复「内存压缩找不到」的困惑）——
  CapsuleUI.hoverOn();
  const procTrigger = document.getElementById('proc-trigger');
  assert.ok(procTrigger, '面板应包含 CPU+内存 悬停触发区');
  procTrigger.dispatchEvent(new dom.window.MouseEvent('mouseenter', { bubbles: true }));
  assert.ok(document.getElementById('pop-sys-toggle'), '弹层应包含「含系统进程」开关');
  calls.statsCb({
    cpu: 10,
    cores: 16,
    mem: { total: 16 * GB, free: 4 * GB, used: 12 * GB },
    ...
    processes: {
      apps: [
        { name: 'WorkBuddy', rawName: 'WorkBuddy', memBytes: 2 * GB, cpuPercent: 3.4, self: false, system: false, status: '运行中', iconLabel: 'W', iconColor: '#0a0' },
        { name: '内存压缩', rawName: 'Memory Compression', memBytes: 1.3 * GB, cpuPercent: 0, self: false, system: true, status: '系统', iconLabel: 'M', iconColor: '#888' },
        { name: 'Windows 服务主机', rawName: 'svchost', memBytes: 0.8 * GB, cpuPercent: 0, self: false, system: true, status: '系统', iconLabel: 'W', iconColor: '#999' },
      ],
      cpuSum: 3,
      memSumBytes: 4.1 * GB,
      cores: 16,
  ...
  let procRows = CapsuleUI.getProcRows();
  assert.strictEqual(procRows.length, 1, '默认应只显示用户应用（自动隐藏系统进程）');
  assert.strictEqual(procRows[0].name, 'WorkBuddy', '默认列表应只剩 WorkBuddy');
  assert.ok(CapsuleUI.getProcGroupText().indexOf('应用 (1)') >= 0, '分组标题应显示应用数');
  assert.strictEqual(CapsuleUI.isShowSystem(), false, '默认不含系统进程');

  // 切换到「含系统进程」→ 出现标为「系统」的行
  CapsuleUI.toggleSystem();
  procRows = CapsuleUI.getProcRows();
  assert.strictEqual(procRows.length, 3, '切换后应显示含系统进程的全部条目');
  assert.strictEqual(CapsuleUI.isShowSystem(), true, '切换后应处于含系统状态');
  const sysRow = procRows.filter((r) => r.system)[0];
  assert.ok(sysRow && sysRow.status === '系统', '系统进程应标记为「系统」');
  assert.ok(sysRow.cls.indexOf('st-sys') >= 0, '系统进程应使用 st-sys 样式类');
  assert.ok(CapsuleUI.getProcGroupText().indexOf('应用与系统 (3)') >= 0, '分组标题应反映当前内容');

  // 再次切换 → 恢复隐藏
  CapsuleUI.toggleSystem();
  assert.strictEqual(CapsuleUI.getProcRows().length, 1, '再次切换应重新隐藏系统进程');

  dom.window.close();
  fs.rmSync(userDataDir, { recursive: true, force: true });

  console.log('PASS  Phase 14: 胶囊悬停资源面板（展开/钉住/实时渲染 CPU·内存·多磁盘）');
  console.log('PASS  Phase 14: 应用占用弹层（系统进程默认隐藏 + 可切换显示并标「系统」）');
});
