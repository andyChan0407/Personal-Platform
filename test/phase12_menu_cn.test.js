...
/* ---------- ② macOS 模板（应用菜单同样中文） ---------- */
const macTemplate = buildMenuTemplate({ appName: '我的待办', isMac: true });
assert.strictEqual(macTemplate.length, 6, 'macOS 应多出一条应用菜单');
assert.strictEqual(macTemplate[0].label, '我的待办', 'macOS 首个菜单应显示应用中文名');
...
try {
  ...
  setupAppMenu({ appName: '我的待办', isMac: false, handlers: {} });
} finally {
