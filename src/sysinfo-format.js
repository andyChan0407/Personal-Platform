...
(function (root, factory) {
  ...
  // 进程弹层：内存以 MB 呈现，保留 1 位小数并加千分位（与图2一致："1,726.8 M"）
  function fmtMB(bytes) {
    const v = toNumber(bytes) / (1024 * 1024);
    const s = v.toLocaleString('en-US', { minimumFractionDigits: 1, maximumFractionDigits: 1 });
    return s + ' M';
  }

  // 进程弹层：CPU 占用百分比，保留 1 位小数（"10.5%"）；0 显示 "0%"
  function fmtProcCpu(pct) {
    const v = toNumber(pct);
    if (v <= 0) return '0%';
    const s = v.toLocaleString('en-US', { minimumFractionDigits: 1, maximumFractionDigits: 1 });
    return s + '%';
  }

  // 进程弹层列合计：CPU 为百分比之和，内存为字节之和转 MB
  function procTotals(processes) {
    const p = processes || {};
    return {
      cpuText: fmtProcCpu(p.cpuSum),
      memText: fmtMB(p.memSumBytes),
    };
  }
